...
classdef spring_particle_set
    ...
    properties
        ...
        orignumpts = 50; % [20][200] number of particles in system
    ...
    methods
        ...
        function obj = spring_particle_set(steps)
        ...
        function obj = initialize(obj,img,cArray)
        ...
        function obj = track_target_step(obj,step,img)
            ...
            if maxb < 0.02 
                ...
                if obj.islost == 1
                    ...
                    obj.scalex = obj.origscalex;
                    obj.scaley = obj.origscaley;
                    obj.targetmethod = 2; % use "robust mean" method
            ...
            for i = 1:obj.numpts
                ...
                while isvalid == 0
                    ...
                    if isvalid == 0 && attempts > 10
                        ...
                        sk1(i,1) = floor(rand([1]) * obj.imgwidth);
                        %sk1(i,2) = randi(obj.imgheight);
                        sk1(i,2) = floor(rand([1]) * obj.imgheight);
                        isvalid = 1;
                    end
                end
            end
         ...
         % step through alg.
        function obj = track_spring_target_step(obj, step, springMate, img)
            if obj.usehsv == 1
                img = rgb2hsv(img);
            end

            %SpringMate is the target vertex of the root particle set
            sX = springMate(1);
            sY = springMate(2);
            ...
            for i = 1:obj.numpts
                ...
                %Calculated distance to root particle set
                targetDistance = sqrt( (obj.sk(i,1) - sX)^2 + (obj.sk(i,2) - sY)^2);
                
                %If the distance is greater than some threshold sT,
                %Adjust probability based on Spring Equation
                if targetDistance > obj.sT
                    %Potential Energy of a Spring: U = 0.5 * k * x^2
                    U = 0.5 * obj.springK * targetDistance^2;
                    adjustedB = b .* (1/U);
                    obj.sk(i,3) = adjustedB;
                end
                
                %If the distance is less than half of the original
                %distance, adjust the probability
                if targetDistance < (0.5 * obj.startDistance)
                   %Code here 
        ...
        %set the colors
        function obj = set_color(obj, cArray)
           disp('Setting Color') 
        end
        
        %Set the spring mate
        function obj = set_startDistance(obj, springMate)
            %SpringMate is the target vertex of the root particle set
            sX = springMate(1);
            sY = springMate(2);
            ...
            %Calculated distance to root particle set
            obj.startDistance = sqrt( (obj.target(1) - sX)^2 + (obj.target(2)- sY)^2);
        end
        ...
        function obj = mark_up(obj,step)
           ...
           % clear previous graphics
            ...
            gcount = 1;
            %obj.graphics(gcount) = plot(obj.oldsk(:,1),obj.oldsk(:,2),obj.colorArray(1:2));
            %gcount = gcount + 1;
            
            %obj.graphics(gcount) = plot(obj.sk(:,1),obj.sk(:,2),obj.colorArray(3:4));
            %gcount = gcount + 1;
            ...
            for i = 1:size(obj.oldtargets)
               obj.graphics(gcount) = plot(obj.oldtargets(i,1),obj.oldtargets(i,2),obj.colorArray(5:6));
               gcount = gcount + 1;
            end

            obj.graphics(gcount) = plot(obj.target(1),obj.target(2),obj.colorArray(7:8)); 
            % drawnow; % bottleneck  
        end
