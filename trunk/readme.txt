    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_brownian_test.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: brownian_test.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * brownian_test.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * brownian_test.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * brownian_test (application created by MATLAB Compiler)
        * run_brownian_test.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * brownian_test.dll (shared library, file extension varies by 
          platform)
        * brownian_test.h (library header file)
    ...
    Windows:
        * brownian_test.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * brownian_test.xml (documentation files)
        * brownian_test.pdb (program database file - if DEBUG option 
          selected)
        * brownian_test.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * brownian_test.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * brownian_test_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * brownian_test.jar (Java class files and compiled M-code)
    ...
    Windows:
        * brownian_test.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * brownian_test_1_0.dll (COM component)
        * brownian_test.xla (optionally created and placed in
