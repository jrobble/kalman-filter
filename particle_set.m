...
classdef particle_set
    ...
    methods
        ...
        function obj = particle_set(steps)
            ...
            obj.posqueue = circular_queue(obj.postostore); % QUEUE
            if nargin > 0
                obj.steps = steps;
            end
        end
        ...
        function obj = track_target_step(obj,step,img)
            ...
            % calculate particle probabilities - bottleneck
            % invoke C++ (must convert objs to structs)
            % img(1:10,1:10,:) % DEBUG
            % obj.sk
            
            warning('OFF','all'); % prevent annoying obj => struct error msg
            calc_particle_probabilities(struct(obj),img); % modifies obj
            warning('ON','all');
